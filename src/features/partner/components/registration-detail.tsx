...
import { APPLICATION_STATUS_LABELS, OPERATIONAL_STATUS_LABELS } from '@/core/constants/partner-registration';
  ...
  return (
    <div className="grid gap-5">
      ...
      {application.activationBlockers.length > 0 && (
        <Card className="border-amber-200 bg-amber-50">
          <CardContent className="space-y-2 py-4">
            <p className="text-sm font-medium text-amber-900">Chưa thể kích hoạt quán</p>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <SummaryChip
          label="Sơ đồ bàn"
          value={application.isTableLayoutConfigured ? 'Đã cấu hình' : 'Chưa cấu hình'}
      ...
      <Card className={SECTION_STYLES.basic}>
        ...
        <CardContent className="grid gap-4">
          <div className="grid gap-3">
            ...
            />
            <InfoRow label="Email đại diện" value={application.representativeEmail || '—'} />
            <InfoRow label="Giờ mở cửa" value={formatWorkingHours(application.workingHours)} />
            <InfoRow label="Giấy phép KD" value={application.businessLicense || '—'} />
          </div>
          {application.businessLicenseImageUrl ? (
            ...
            <p className="text-sm text-muted-foreground">Chưa có file giấy phép đính kèm.</p>
          )}
        </CardContent>
      </Card>
      ...
      <Card className={SECTION_STYLES.infra}>
        ...
        <CardContent className="grid gap-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <InfoRow label="Số bàn công cộng" value={application.numberOfTables} />
            <InfoRow label="Phòng riêng" value={application.numberOfPrivateRooms} />
            <InfoRow
              label="Sơ đồ bàn"
              value={application.isTableLayoutConfigured ? 'Đã cấu hình' : 'Chưa cấu hình'}
            />
          ...
          {application.spaceImageUrls.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground">Chưa có ảnh không gian đính kèm.</p>
          )}
        </CardContent>
      </Card>
      ...
      <Card className={SECTION_STYLES.catalog}>
        ...
        <CardContent className="grid gap-3">
          <InfoRow label="Tổng số game sở hữu" value={application.numberOfGamesOwned} />
          <InfoRow label="Game phổ biến" value={application.popularGamesList || '—'} />
        </CardContent>
      </Card>

      <Card className={SECTION_STYLES.services}>
        <CardHeader>
          <CardTitle className="text-base">Dịch vụ & Mô hình vận hành</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-3">
          <InfoRow label="Có Game Master" value={application.hasGameMaster ? 'Có' : 'Không'} />
          <InfoRow
      ...
      <Card className={SECTION_STYLES.audit}>
        ...
        <CardContent className="grid gap-3">
          ...
          />
          <InfoRow label="Người nộp đơn" value={application.submittedByUsername ?? '—'} />
          <InfoRow label="Admin duyệt" value={application.reviewedByAdminUsername ?? '—'} />
          <InfoRow label="Thời gian nộp" value={formatApiDate(application.submittedAt)} />
          <InfoRow label="Thời gian xem xét" value={formatApiDate(application.reviewedAt)} />
          <InfoRow label="Thời gian duyệt" value={formatApiDate(application.approvedAt)} />
          <InfoRow
            label="Cập nhật hồ sơ vận hành"
            value={formatApiDate(application.operationalProfileUpdatedAt)}
          />
          <InfoRow label="Cập nhật gần nhất" value={formatApiDate(application.updatedAt)} />
          <InfoRow label="Mã quán đã tạo" value={application.createdCafeId ?? '—'} />
          <InfoRow label="Mã manager đã tạo" value={application.createdManagerUserId ?? '—'} />
          {application.submittedByUserId && (
            <InfoRow label="Mã người nộp" value={application.submittedByUserId} />
          )}
          {application.reviewedByAdminId && (
            <InfoRow label="Mã admin duyệt" value={application.reviewedByAdminId} />
          )}
        </CardContent>
      </Card>
