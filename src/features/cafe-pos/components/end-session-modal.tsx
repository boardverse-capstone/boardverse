...
export function EndSessionModal({
  ...
  return (
    ...
    >
      {/* MODAL CONTAINER */}
      <div
        className="relative max-w-md w-full overflow-hidden rounded-2xl border-2 border-rose-400 bg-gradient-to-br from-rose-50 via-pink-50 to-fuchsia-50 shadow-[6px_6px_0_rgba(244,63,94,0.4)]"
      ...
      >
        ...
        {/* LED corner dots */}
        <span className="pointer-events-none absolute -left-0.5 -top-0.5 size-2 animate-pulse rounded-full bg-rose-500 shadow-[0_0_10px_currentColor]" />
        <span className="pointer-events-none absolute -right-0.5 -bottom-0.5 size-2 animate-pulse rounded-full bg-fuchsia-500 shadow-[0_0_10px_currentColor] [animation-delay:0.4s]" />
        ...
        {/* HEADER */}
        <div className="relative flex items-center justify-between border-b-2 border-rose-300/70 bg-gradient-to-r from-rose-100 via-pink-100 to-fuchsia-100 px-5 py-3">
          <div className="flex items-center gap-3">
            {/* Icon với glow */}
            <div className="relative p-2.5 rounded-xl border-2 border-rose-400 bg-gradient-to-br from-rose-400 to-rose-600 shadow-[inset_0_-2px_0_rgba(0,0,0,0.2),0_0_12px_rgba(244,63,94,0.5)]">
              <LogOut className="w-5 h-5 text-white" />
              <span className="absolute -right-1 -top-1 size-2 animate-pulse rounded-full bg-yellow-400 shadow-[0_0_8px_currentColor]" />
            </div>
            <div>
              <h3 className="font-mono text-sm font-extrabold uppercase tracking-widest text-rose-950">
                ► Trả bàn — kết thúc giờ chơi
              </h3>
              <p className="font-mono text-[10px] font-bold uppercase tracking-widest text-rose-700">
                <span className="text-rose-400">▸</span> Bàn{" "}
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-xl border-2 border-rose-300 bg-white p-1.5 font-mono text-rose-500 shadow-[2px_2px_0_rgba(244,63,94,0.3)] transition-all hover:border-rose-500 hover:bg-rose-50"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="relative space-y-2 p-4">
          {/* Row 1: Thời gian */}
          <div className="flex items-center justify-between rounded-xl border-2 border-rose-300 bg-white/80 px-4 py-3 shadow-[inset_0_-2px_0_rgba(0,0,0,0.06)]">
            <span className="flex items-center gap-2 font-mono text-[11px] font-bold uppercase tracking-widest text-rose-700">
              <span className="size-1.5 rounded-full bg-rose-500 shadow-[0_0_6px_currentColor] animate-pulse" />
              <Clock className="w-3.5 h-3.5" /> Thời gian đã chơi:
            </span>
            <span className="font-mono text-lg font-extrabold uppercase tracking-wide text-rose-950 [text-shadow:1px_1px_0_rgba(255,255,255,0.8)]">
          ...
          <div className="flex items-center justify-between rounded-xl border-2 border-rose-300 bg-white/80 px-4 py-3 shadow-[inset_0_-2px_0_rgba(0,0,0,0.06)]">
            <span className="flex items-center gap-2 font-mono text-[11px] font-bold uppercase tracking-widest text-rose-700">
              <span className="size-1.5 rounded-full bg-sky-500 shadow-[0_0_6px_currentColor] animate-pulse [animation-delay:0.3s]" />
              <Users className="w-3.5 h-3.5" /> Tổng số khách:
            </span>
            <span className="font-mono text-base font-extrabold uppercase tracking-wide text-rose-950">
              {readPresentCount(session) ?? session.members?.length ?? 0}{" "}
              <span className="text-rose-500">người</span>
            </span>
          </div>

          {/* Row 3: Host */}
          <div className="flex items-center justify-between rounded-xl border-2 border-rose-300/70 bg-white/60 px-4 py-2.5 shadow-[inset_0_-1px_0_rgba(0,0,0,0.05)]">
            <span className="font-mono text-[10px] font-bold uppercase tracking-widest text-rose-600">
              <span className="text-rose-400 mr-1">▸</span>Người phụ trách:
            </span>
            <span className="font-mono text-xs font-extrabold uppercase tracking-wide text-rose-900 truncate max-w-[180px]">
              {session.hostName || "—"}
            </span>
          </div>
        </div>
        ...
        {/* FOOTER ACTIONS */}
        <div className="relative flex justify-end gap-2 border-t-2 border-rose-300/70 bg-gradient-to-r from-rose-100/60 to-fuchsia-100/60 px-5 py-3">
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            ...
            disabled={loading}
            onClick={handleEnd}
            className="relative inline-flex h-9 items-center justify-center gap-1.5 rounded-xl border-2 border-rose-600 bg-gradient-to-b from-rose-500 to-rose-700 px-5 font-mono text-[11px] font-extrabold uppercase tracking-widest text-white shadow-[inset_0_-2px_0_rgba(0,0,0,0.25),3px_3px_0_rgba(0,0,0,0.15)] transition-all hover:from-rose-400 hover:to-rose-600 disabled:cursor-not-allowed disabled:from-gray-400 disabled:to-gray-500 disabled:shadow-[inset_0_-2px_0_rgba(0,0,0,0.2)]"
          >
            {loading ? (
              <>
